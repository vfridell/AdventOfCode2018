    ...
    public class Ten
    {
        ...
        public static void Part1()
        {
            ...
            do
            {
                checkTime1++;
                checkTime2++;
                newVectors1 = GetVectorsAtTime(vectors, checkTime1);
                newVectors2 = GetVectorsAtTime(vectors, checkTime2);
                boundingBoxSize1 = GetBoundingBoxMaxSide(newVectors1);
                boundingBoxSize2 = GetBoundingBoxMaxSide(newVectors2);
            } while (boundingBoxSize1 >= boundingBoxSize2);

            PrintDisplay(newVectors1);
        }

        private static void PrintDisplay(List<Vector> vectors)
        {
            int boundingBoxSize = GetBoundingBoxMaxSide(vectors);
            for(int r=0; r<=boundingBoxSize; r++)
            {
                for(int c=0;c<=boundingBoxSize; c++)
                {
                    if (vectors.FirstOrDefault(v => v.Position.X == c && v.Position.Y == r) != null) Console.Write("X");
                    else Console.Write(" ");
                }
                Console.WriteLine();
            }
        ...
        public static List<Vector> GetVectorsAtTime(List<Vector> vectors, int time)
        {
            ...
            for (int i = 0; i < vectors.Count; i++) adjustedVectors.Add(vectors[i].GetVectorAt(time));
...
"position=<-3,  6> velocity=< 2, -1>",
        ...
        {
            "position=< 11118, -32416> velocity=<-1,  3>",
