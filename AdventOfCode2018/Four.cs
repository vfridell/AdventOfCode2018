...
namespace AdventOfCode2018
{
    public class Four
    {
        public class GuardStats
        {
            ...
            public int SleepiestMinuteCount { get; set; }
            public int TotalMinutesAsleep { get; set; }
            public int[] TotalMinutesSleepingAt { get; set; } =new  int[60];
        }
        ...
        public class GuardShift
        {
            ...
            public List<DateTime> FallAsleep { get; set; } = new List<DateTime>();
            public List<DateTime> WakeUp { get; set; } = new List<DateTime>();
            public bool StartedBeforeMidnight => Start.Hour == 23;
            public TimeSpan TotalTimeSpan
            {
                get
                {
                    if (Start.Hour < 23)
                        return new DateTime(Start.Year, Start.Month, Start.Day, 01, 00, 00) - Start;
                    else
                    {
                        DateTime nextDay = Start.AddDays(1);
                        return new DateTime(nextDay.Year, nextDay.Month, nextDay.Day, 01, 00, 00) - Start;
                    }
                }
            }
            public TimeSpan SleepTimeSpan { get; set; }
            public TimeSpan AwakeTimeSpan { get; set; }
            public List<int> Sleeping = new List<int>();

            public void CalcSpans()
            {
                AwakeTimeSpan = new TimeSpan(0);
                SleepTimeSpan = new TimeSpan(0);
                if (!FallAsleep.Any())
                {
                    AwakeTimeSpan = TotalTimeSpan;
                    return;
                }

                for (int i = 0; i < WakeUp.Count; i++)
                {
                    for (int m = FallAsleep[i].Minute; m < WakeUp[i].Minute; m++) Sleeping.Add(m);
                    SleepTimeSpan += WakeUp[i] - FallAsleep[i];
                }

                AwakeTimeSpan = TotalTimeSpan - SleepTimeSpan;
            }

            public bool Asleep(int minute)
            {
                if (!FallAsleep.Any()) return false;
                return Sleeping.Contains(minute);
            }
        }

        public static string timeRegexString = @"\[1518-([0-9]{2})-([0-9]{2}) ([0-9]{2}):([0-9]{2})\] ";
        public static Regex shiftBeginRegex = new Regex(timeRegexString + @"Guard #([0-9]+) begins shift");
        public static Regex wakeBeginRegex = new Regex(timeRegexString + @"wakes up");
        public static Regex sleepBeginRegex = new Regex(timeRegexString + @"falls asleep");
        public static void Part1()
        {
            ...
            {
                if(shiftBeginRegex.IsMatch(input))
                {
                    guardShift?.CalcSpans();
                    MatchCollection m = shiftBeginRegex.Matches(input);
                    int guardId = int.Parse(m[0].Groups[5].Value);
                    guardShift = new GuardShift()
                    {
                        Id = guardId,
                        Start = ParseTimestamp(m),
                    };
                    guardShifts.Add(guardShift);
                }
                else if(sleepBeginRegex.IsMatch(input))
                {
                    MatchCollection m = sleepBeginRegex.Matches(input);
                    guardShift.FallAsleep.Add(ParseTimestamp(m));
                }
                else if(wakeBeginRegex.IsMatch(input))
                {
                    MatchCollection m = wakeBeginRegex.Matches(input);
                    guardShift.WakeUp.Add(ParseTimestamp(m));
                }
                else
                {
            ...
            Dictionary<int, GuardStats> Stats = new Dictionary<int, GuardStats>();

            int SleepingestGuardId = 0;
            int MaxMinutesSleeping = 0;
            foreach(GuardShift g in guardShifts)
            {
                if (!Stats.ContainsKey(g.Id)) Stats[g.Id] = new GuardStats();
                Stats[g.Id].TotalMinutesAsleep += g.SleepTimeSpan.Minutes;
                if(Stats[g.Id].TotalMinutesAsleep > MaxMinutesSleeping)
                {
                    SleepingestGuardId = g.Id;
                    MaxMinutesSleeping = Stats[g.Id].TotalMinutesAsleep;
                }
            }

            var groups = guardShifts.Where(s => s.Id == SleepingestGuardId)
                        .SelectMany(g => g.Sleeping)
                        .GroupBy(l => l);
            var sleepiestMinuteGroup = groups.OrderByDescending(g => g.Count()).First();
            Console.WriteLine($"Guard {SleepingestGuardId} sleeps the most ({MaxMinutesSleeping} minutes) during the {sleepiestMinuteGroup.Key} minute");
            Console.WriteLine($"Calculated answer is: {SleepingestGuardId * sleepiestMinuteGroup.Key}");
        }
        ...
        public static void Part2()
        {
            ...
            foreach (GuardShift g in guardShifts)
            {
                if (!Stats.ContainsKey(g.Id)) Stats[g.Id] = new GuardStats();
                Stats[g.Id].Id = g.Id;
                foreach (int min in g.Sleeping)
                {
                    Stats[g.Id].TotalMinutesSleepingAt[min]++;
                }
            }

            Tuple<int,int,int> theSleepingestData = Stats.Values
                        .Select(g => new Tuple<int, int, int>(g.TotalMinutesSleepingAt.ToList().IndexOf(g.TotalMinutesSleepingAt.Max()), g.TotalMinutesSleepingAt.Max(), g.Id))
                        .OrderByDescending(t => t.Item2)
                        .First();
            
            Console.WriteLine($"Guard {theSleepingestData.Item3} sleeps the most frequently ({theSleepingestData.Item2} times) during the {theSleepingestData.Item1} minute");
            Console.WriteLine($"Calculated answer is: {theSleepingestData.Item3 * theSleepingestData.Item1}");
        }

        public static DateTime ParseTimestamp(MatchCollection m)
        {
            int month = int.Parse(m[0].Groups[1].Value);
            int day = int.Parse(m[0].Groups[2].Value);
            int hour = int.Parse(m[0].Groups[3].Value);
            int minute = int.Parse(m[0].Groups[4].Value);
            return new DateTime(1518, month, day, hour, minute, 0);
        }

        public static List<string> InputTest = new List<string> { "[1518-11-01 00:00] Guard #10 begins shift",
